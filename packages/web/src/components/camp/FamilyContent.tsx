...
function SsnRow({ value, campCode, sensitiveKey, label, isAdmin }: { value?: string; campCode?: string | null; sensitiveKey: string; label: string; isAdmin: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[10px] text-gray-400 uppercase tracking-wide">{L('common.idNumber')}</span>
        ...
        <div className="border-t border-gray-100 divide-y divide-gray-100">
            ...
            <div key={parent.id} className="px-4 py-3 space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-blue-100 text-blue-700">
                  {L('students.parents')}{family.parents.length > 1 ? ` ${idx + 1}` : ''}
                </span>
                <span className="text-sm font-bold text-gray-800">{parent.name}</span>
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                <Row label={L('students.contact')}    value={parent.phone} />
                <Row label={L('students.region')}      value={parent.region} />
                <Row label={L('students.nativeTeacherClass2')} value={
                  parent.nativeEnglish && parent.nativeEnglish !== '신청 X' && parent.nativeEnglish !== '-'
                    ? parent.nativeEnglish : undefined
                } />
                <Row label={L('students.email')}    value={parent.email} />
                ...
                <Row label={L('students.passportName')}  value={parent.passportName} />
                <Row label={L('students.passportNo')}  value={parent.passportNumber} />
                <Row label={L('students.passportExpiry')}  value={parent.passportExpiry !== '0000.00.00' ? parent.passportExpiry : undefined} />
                <Row label={L('students.roomNo')}    value={parent.roomNumber} />
                <Row label={L('profile.address')}      value={parent.address}  wide />
                <Row label={L('students.other')}      value={parent.notes}    wide />
              </div>
            </div>
          ))}
          ...
          {family.students.map((student, idx) => (
            <div key={student.id} className="px-4 py-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                <Row label={L('students.gradeGender')} value={`${student.grade} · ${student.gender === 'M' ? L('students.male') : L('students.female')}`} />
                <Row label={L('students.studentId')}   value={student.id} />
                <Row label={L('students.parentContact')} value={student.parentPhone} />
                <Row label={L('students.registeredVia')}   value={student.registrationSource} />
                ...
                <Row label={L('students.passportName')}  value={student.passportName} />
                <Row label={L('students.passportNo')}  value={student.passportNumber} />
                <Row label={L('students.passportExpiry')}  value={student.passportExpiry !== '0000.00.00' ? student.passportExpiry : undefined} />
                <Row label={L('students.healthInfo')}  value={student.medication} wide />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
        <div className="flex items-center gap-2">
          <h2 className="text-base font-semibold text-gray-800">{L('students.familyList')}</h2>
          {!loading && (
            <span className="text-xs text-gray-400">
              {filtered.length}{L('students.familiesStudents')} {totalStudents}{L('common.people2')}
            </span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-400">
            ...
            <span className="text-sm">{L('students.loadingFamilyData')}</span>
          </div>
        ) : filtered.length === 0 ? (
            ...
            return (
              <div key={type} className="space-y-2">
                ...
                <div className={`flex items-center justify-between px-3 py-2 rounded-lg ${style.sectionBg} ${style.sectionText}`}>
                  ...
                  <span className="text-xs opacity-80">{list.length}{L('students.familiesStudents')} {list.reduce((s, f) => s + f.students.length, 0)}{L('common.people2')}</span>
                </div>
                {/* 카드 목록 */}
                {list.map(family => (
