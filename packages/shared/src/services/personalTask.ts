/**
 * 개인 업무 (본인만 보는 할 일) — web·mobile 공용. 각 앱은 자기 db·auth 로 한 번 만들어 쓴다.
 */
import {
  type Firestore,
  collection,
...
import type { PersonalTask } from '../types/camp';
...
export function createPersonalTaskService(db: Firestore, auth: Pick<Auth, 'currentUser'>) {
  // 개인 업무 생성
  ...
  const getOverduePersonalTasks = async (
    ownerId: string,
    campCode: string
  ...
  const getPersonalTasksByDate = async (
    ownerId: string,
    campCode: string,
    date: Date
  ): Promise<PersonalTask[]> => {
  ...
  const getPersonalTaskDatesInMonth = async (
    ownerId: string,
    campCode: string,
    year: number,
    month: number
  ...
  const getPersonalTasksInMonth = async (
    ownerId: string,
    campCode: string,
    year: number,
    month: number
  ...
  const getPersonalTasksByGroupId = async (groupId: string): Promise<PersonalTask[]> => {
    try {
  ...
  const updatePersonalTaskGroup = async (
    groupId: string,
    ownerId: string,
    campCode: string,
    updates: {
  ...
  // 그룹 개인 업무 전체 삭제
  const deletePersonalTaskGroup = async (groupId: string): Promise<void> => {
    try {
      const groupTasks = await getPersonalTasksByGroupId(groupId);
      const batch = writeBatch(db);
      groupTasks.forEach(task => {
  ...
  // 단일 개인 업무 조회
  const getPersonalTaskById = async (taskId: string): Promise<PersonalTask | null> => {
    try {
  ...
  // 개인 업무 완료 토글
  const togglePersonalTaskCompletion = async (taskId: string, currentState: boolean): Promise<void> => {
    try {
