...
export function MyEscortPanel({ records, myName, onOpen }: { records: PatientRecord[]; myName?: string | null; onOpen: (id: string) => void }) {
  ...
  return (
    <View style={{ marginHorizontal: 12, marginTop: 10, marginBottom: 4, borderRadius: 12, borderWidth: 2, borderColor: '#fdba74', backgroundColor: '#fff7ed', padding: 10, gap: 8 }}>
      <Text style={{ fontSize: 12, fontWeight: '800', color: '#9a3412' }}>🚑 내가 인솔할 학생 {mine.length}명 — 병원 접수용 정보</Text>
      {mine.map(({ r, v }) => (
        <TouchableOpacity key={r.id} activeOpacity={0.8} onPress={() => onOpen(r.id)}
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#111827' }}>
              {r.studentName}
              <Text style={{ fontSize: 11, fontWeight: '400', color: '#6b7280' }}>  {[r.grade, r.className, r.roomNumber && `${r.roomNumber}호`].filter(Boolean).join(' · ')}</Text>
            </Text>
            <Text style={{ fontSize: 10, fontWeight: '700', color: '#fff', backgroundColor: v.hospitalStatus === '내원예정' ? '#f97316' : '#22c55e', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, overflow: 'hidden' }}>{v.hospitalStatus}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Text style={{ fontSize: 11, color: '#6b7280' }}>주민번호</Text>
            <EscortSsn recordId={r.id} auto />
          </View>
          {!!v.hospitalName && <Text style={{ fontSize: 11, color: '#374151' }}>병원: {v.hospitalName}</Text>}
          {(!!v.departureTime || !!v.transportSlot) && <Text style={{ fontSize: 11, color: '#374151' }}>{[v.departureTime && `출발 ${v.departureTime}`, v.transportSlot].filter(Boolean).join(' · ')}</Text>}
          {!!r.symptom && <Text style={{ fontSize: 11, color: '#374151' }}>증상: {r.symptom}</Text>}
        </TouchableOpacity>
      ))}
    </View>
  );
