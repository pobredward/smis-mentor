...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: 'SMIS 가이드 페이지',
      description: '가이드를 확인하세요',
    };
  }
}

export default async function ShareGuidePage({ params }: PageProps) {
  const { itemId } = await params;
          ...
          <div className="max-w-5xl mx-auto px-4 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">SMIS Mentor 가이드</p>
              </div>
            </div>
          </div>
        </div>
