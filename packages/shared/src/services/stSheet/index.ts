 ...
 * 이제 웹과 같이: F 캠프는 familySTSheetCache, 실제 데이터 없으면 빈 목록(임시 데이터는 설정을 켰을 때만).
 */
...
// 임시(샘플) 학생 데이터 — campSettings.useTemporaryData 를 켰을 때만 보여 준다
export const getTemporaryStudents = (campCode: CampCode): STSheetStudent[] => {
...
export function createStSheetService(db: Firestore, opts: { sync: (campCode: CampCode) => Promise<SyncSTSheetResponse> }) {
  const getTemporaryData = getTemporaryStudents;
  const service = {
    ...
    isTemporaryData: async (campCode: CampCode = 'E27'): Promise<boolean> => {
      try {
        const useTemporaryData = await service.getUseTemporaryDataSetting(campCode);
    ...
    getUseTemporaryDataSetting: async (campCode: CampCode = 'E27'): Promise<boolean> => {
      try {
        ...
        const hasReal = await service.hasRealData(campCode);
