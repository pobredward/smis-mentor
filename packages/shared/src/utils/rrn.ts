/**
 * 주민등록번호 앞자리·뒷자리로 성별·나이·생년월일 계산 (web·mobile 공용, 예전 각 앱의 utils/userUtils)
 */
...
export const getBirthDateFromRRN = (rrnFront: string, rrnLast: string): Date | null => {
  ...
  try {
    return new Date(fullBirthYear, month - 1, day); // JavaScript Date는 월이 0부터 시작
  } catch (error) {
    return null;
  }
};
...
/**
 * 주민등록번호로부터 성별과 나이, 생년월일 정보를 포함한 객체를 반환합니다.
 */
export const getUserInfoFromRRN = (rrnFront: string, rrnLast: string) => {
  return {
    ...
    birthDate: getBirthDateFromRRN(rrnFront, rrnLast),
  };
};
