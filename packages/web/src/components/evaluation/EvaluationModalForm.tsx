...
export default function EvaluationModalForm({
  ...
  const loadCriteriaTemplates = async () => {
    try {
      setIsLoading(true);
      const templates = await EvaluationCriteriaService.getCriteriaByStage(db, evaluationStage);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await EvaluationService.createEvaluation(db, formData, '', evaluatorName, '관리자');
