...
export const COLLECTIONS: Record<string, CollectionSpec> = {
  ...
  evaluations: {
    name: 'evaluations',
    description: '지원자 단계별 평가. MCP 로 쓴 평가는 실행한 관리자 이름으로 바로 확정 저장된다(관리자가 Claude 등에서 검토한 뒤 올리므로 초안 단계 없음). 수정·삭제는 본인이 쓴 평가만. 저장 후 지원자 평가 요약(users.evaluationSummary)을 서버가 다시 계산한다.',
    read: 'admin',
    write: { ops: ['create', 'update', 'delete'] },
    scope: { kind: 'none' },
    idOnCreate: 'auto',
    fields: {
      ...
      evaluatorName: str('평가자 이름 (서버 설정 — 실행한 관리자 이름)'),
      evaluatorRole: str('평가자 역할 (서버 설정)'),
      isFinalized: bool('확정 여부 (MCP 로 쓴 평가도 true)'),
      isVisible: bool('지원자 공개 여부 (앱과 같이 기본 false)'),
      aiDraft: bool('(폐지) 예전 AI 초안 표시 — 더 이상 쓰지 않음'),
    ...
    // 실행한 관리자 명의로 바로 확정 (앱에서 관리자가 직접 쓴 평가와 같은 형태)
    forcedOnCreate: (viewer) => ({
      evaluatorId: viewer.uid,
      evaluatorName: viewer.name,
      evaluatorRole: '관리자',
      isFinalized: true,
      isVisible: false,
