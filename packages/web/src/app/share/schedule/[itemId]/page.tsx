...
export default async function ShareSchedulePage({ params }: PageProps) {
  const { itemId } = await params;
  ...
  try {
    ...
    // 페이지가 없거나 일정 카테고리가 아닌 경우
    if (!page || page.category !== 'schedule') {
      notFound();
    }
          ...
          <div className="max-w-5xl mx-auto px-4 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">SMIS Mentor 일정</p>
              </div>
            </div>
          </div>
        </div>
