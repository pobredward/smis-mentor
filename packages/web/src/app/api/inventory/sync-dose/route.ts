...
/**
 * POST /api/inventory/sync-dose  { recordId, campCode? }
 * 환자 기록의 약 복용 기록을 재고 원장과 맞춘다 (여러 번 호출해도 안전)
 */
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as { recordId?: string; campCode?: string };
  const recordId = String(body.recordId ?? '');
  if (!recordId || recordId.includes('/')) return NextResponse.json({ error: 'recordId가 필요합니다.' }, { status: 400 });
  try {
    const changed = await reconcileDoseLedger(recordId, body.campCode);
