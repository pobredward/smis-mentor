...
interface ArrivalStudent extends STSheetStudent {
  arrivalGroup?: string;
...
export default function ArrivalContent() {
  ...
  const [students, setStudents] = useState<ArrivalStudent[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedGroup, setSelectedGroup] = useState<string | null>(null);
  const [selectedStudent, setSelectedStudent] = useState<ArrivalStudent | null>(null);
  ...
  const handleSelectStudent = useCallback(async (student: ArrivalStudent) => {
    if (!campCode) return;
    const override = await placementOverrideService.getOverride(campCode, student.studentId);
    setSelectedStudent(placementOverrideService.mergeOverride(student, override) as ArrivalStudent);
  }, [campCode]);
  ...
  const loadAllStudents = useCallback(async () => {
    ...
    try {
      ...
      setStudents(data as ArrivalStudent[]);
      ...
      logger.error('❌ [ArrivalContent] 학생 목록 로드 실패:', error);
  ...
  // 퇴소공항조별 그룹화
  const groupedByArrivalGroup = useMemo(() => {
    return students.reduce((acc, student) => {
      const group = student.arrivalGroup?.trim() || '미배정';
      if (!acc[group]) acc[group] = [];
      acc[group].push(student);
      return acc;
    }, {} as Record<string, ArrivalStudent[]>);
  }, [students]);

  // 각 조의 인솔자 추출
  const instructorMap = useMemo(() => {
    const map: Record<string, string> = {};
    Object.keys(groupedByArrivalGroup).forEach(group => {
      const groupStudents = groupedByArrivalGroup[group];
      const instructor = groupStudents.find(s => s.arrivalInstructor)?.arrivalInstructor || '';
      map[group] = instructor;
    });
    return map;
  }, [groupedByArrivalGroup]);
  ...
  const sortedGroups = useMemo(() => {
    const keys = Object.keys(groupedByArrivalGroup);
  ...
  const sortStudents = (list: ArrivalStudent[]): ArrivalStudent[] => {
  ...
  const displayStudents = searchQuery.trim()
    ...
    : selectedGroup
    ? sortStudents(groupedByArrivalGroup[selectedGroup] || [])
    : [];
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">{L('students.loadingDepartureList')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!activeJobCodeId) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4">
        ...
        <p className="text-sm text-gray-600">{L('students.toViewThatCampS')}</p>
      </div>
    );
  }
  ...
  if (campType !== 'EJ') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4">
        ...
        <p className="text-sm font-medium text-gray-600">{L('students.theDepartureListIsOnly')}</p>
      </div>
    );
  }

  // 퇴소공항조 데이터가 없는 경우
  const hasGroupData = students.some(s => s.arrivalGroup);
  if (!hasGroupData && students.length > 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4">
        ...
        <p className="text-sm font-medium text-gray-600">{L('students.noDepartureAirportGroupData')}</p>
        <p className="text-xs text-gray-400 mt-1">{L('students.addTheAndColumnsTo')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        <h1 className="text-lg font-semibold text-gray-900">{L('students.departureList')}</h1>
              ...
              return (
                <button
                  ...
                  className={`px-2 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors flex flex-col items-center ${
                    isSelected
                      ? isUnclassified ? 'bg-gray-500 text-white' : 'bg-orange-500 text-white'
                      : isUnclassified ? 'bg-gray-200 text-gray-500 hover:bg-gray-300' : 'bg-orange-100 text-orange-700 hover:bg-orange-200'
                  }`}
                >
                  <span>{dataLabel(group)}</span>
                  {instructor ? (
                    <span className={`text-[10px] font-normal mt-0.5 ${isSelected ? 'text-orange-100' : 'text-orange-600'}`}>
                      {instructor}
                    </span>
                  ) : isUnclassified ? (
                    <span className={`text-[10px] font-normal mt-0.5 ${isSelected ? 'text-gray-200' : 'text-gray-400'}`}>
                      {groupedByArrivalGroup[group].length}{L('common.people2')}
                    </span>
                  ) : null}
                </button>
              );
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {/* 퇴소 정보 요약 */}
                    {(selectedStudent.arrivalGroup || selectedStudent.arrivalInstructor) && (
                      <div className="mt-3 w-full bg-orange-50 rounded-xl p-3 border border-orange-100">
                        {selectedStudent.arrivalGroup && (
                          <p className="text-xs text-orange-700 font-semibold text-center">{L('students.departure2')} {selectedStudent.arrivalGroup}</p>
                        )}
                        {selectedStudent.arrivalInstructor && (
                          <p className="text-xs text-orange-600 text-center mt-0.5">{L('students.escort')} {selectedStudent.arrivalInstructor}</p>
                        )}
                      </div>
                    )}
                  </>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex flex-col items-center mb-4">
                        ...
                        {/* 퇴소 정보 요약 (모바일) */}
                        {(selectedStudent.arrivalGroup || selectedStudent.arrivalInstructor) && (
                          <div className="mt-3 w-full max-w-xs bg-orange-50 rounded-xl p-3 border border-orange-100">
                            {selectedStudent.arrivalGroup && (
                              <p className="text-xs text-orange-700 font-semibold text-center">{L('students.departure2')} {selectedStudent.arrivalGroup}</p>
                            )}
                            {selectedStudent.arrivalInstructor && (
